...
const AdminMain = () => {
  ...
  useEffect(() => {
    ...
    .get(base()+'/applications', {
      ...
      for(let i=0;i<data.length;i++) {
        if(data[i].application_verdict === "Admin_PENDING")
        Rpending.push(data[i]);
        else if(data[i].application_verdict === "APPROVED")
        Rapproved.push(data[i]);
        else if(data[i].application_verdict === "Admin_REJECTED")
  ...
  return (
    <section className="profile-main">
      <Row className="mb-3">
        <Col md={5} lg={6} sm={12}>
          <AdminFeed data={{pending,approved,rejected}} />
        </Col>
        <Col md={7} lg={6} sm={12}>
          <AdminTable
